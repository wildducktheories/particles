...
// generated by the process will return true if its Read() method is called.
//
type process struct {
	...
	probability float64  // the probability that an observation generated by the process is true
}
...
// rule and each token that matches is becomes a token in the output process.
// The probability of the new process generating a particle with 2 up-quarks
// is calculated from the frequency of 2 up-quark tokens in the output process
...
// sort the input slice. if every element with an odd index
// is equal the preceding element, then reduce returns a new
// slice with the duplicates removed, otherwise it returns
